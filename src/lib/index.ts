...
export const getSupportedFileFormats = async (): Promise<IFormatSupport[]> =>
	Promise.all(
		FILE_FORMATS.map(async extension => {
			/** @ts-ignore TS, default values are fine for props that might not exist... */
			const {mimeType, compressible = false, transparency = false} = formatMetadata[extension]
			const input = await isInputSupported(extension)
			const output = isOutputSupported(extension)

			return {extension, mimeType, compressible, transparency, input, output}
		})
	)
